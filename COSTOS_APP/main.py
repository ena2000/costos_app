...
class App:
    ...
    def obtener_resultados(self):
        ...
        # 6. Cálculo del Gran Total para el final del pegado
        # Sumamos todos los valores numéricos de las interfaces
        total_final = self.maquinaria_ui.total_costo_maquinaria + \
                      self.mano_obra_ui.get_totales()["costo_mo_total"] + \
                      self.mano_obra_ui.get_totales()["costo_ins_total"] + \
                      self.mano_obra_ui.get_totales()["viaticos"] + \
                      self.mano_obra_ui.get_totales()["cif"] + \
                      getattr(self.tinta_ui, 'total_gasto', 0.0) + \
                      getattr(self.materiales_ui, 'total_costo', 0.0)
        
        # Agregamos la fila del Total General (con tabs para que caiga en la columna de costos)
        #res += f"\n\t\tTOTAL GENERAL\t{self.formato_coma(total_final)}"
    ...
    def copiar_resultado(self):
        ...
        messagebox.showinfo("Copiado", "Todos los datos copiados.")
